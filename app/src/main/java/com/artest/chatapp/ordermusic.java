...
public class ordermusic extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pd = new ProgressDialog(ordermusic.this);
        ...
        RequestQueue rQueue = Volley.newRequestQueue(ordermusic.this);
        rQueue.add(request);

        usersList.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            @Override
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                MusicDetails.songname = al.get(position);
                startActivity(new Intent(ordermusic.this, Room.class));
            }
        });
    }
    ...
    public void doOnSuccess(String s){
        ...
        try {
            ...
            while(i.hasNext()){
                key = i.next().toString();
                singer=obj.getJSONObject(key).getString("singer");
                al.add(key);
                bl.add(singer);
                totalUsers++;
            }
